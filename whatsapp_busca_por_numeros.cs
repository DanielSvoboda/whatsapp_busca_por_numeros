...
namespace whatsapp_busca_por_numeros
{
    public partial class whatsapp_busca_por_numeros : Form
    {
        ...
        {
            double num_inicio  = Convert.ToDouble(textBox_DDD.Text + textBox_num_inicio.Text);
            double num_fim     = Convert.ToDouble(textBox_DDD.Text + textBox_num_fim.Text);
        ...
        private async Task procurar (double num_inicio, double num_fim)
        {
            ...
            {
                for (double numero = num_inicio; numero <= num_fim; numero++)
                {
                    ...
                    var html_raw = webClient_html.DownloadString( url + numero );                    
                    
                    int nun_inicio_nome = html_raw.IndexOf("_9vd5 _9scb _9scr");//Numero onde inicia as palavras

                    if (nun_inicio_nome.ToString() != "-1") //se não achar é pq não exite ou é privado
                    {
                        int nun_final_nome = html_raw.IndexOf("</h3><a href");  //Numero onde termina as palavras
                        string nome = html_raw.Substring(nun_inicio_nome + 19,  //Corta o conteudo entre os 2 'Numero'
                            nun_final_nome - nun_inicio_nome - 19);

                        if (checkBox_Foto.Checked == true)
                        {
                        ...
                        {
                            await add_datagrid(null, nome, numero);
                        }
                        quantidade_localizado++;
                    }
                }
            }
            else
        ...
        private async Task add_datagrid(Image imagem,string nome, double numero)
        {
